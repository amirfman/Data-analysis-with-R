...
parameter<- c( e=0.6 ,      #Remineralization rate
               mu=0.1 ,     #Phytoplankton maximum growth rate
...
parameter2<- c( e=0.6 ,      #Remineralization rate
               mu=0.5 ,     #Phytoplankton maximum growth rate
               kN=0.3 ,     #Half-saturation constant
               mp=0.06 ,    #Phytoplankton mortality
               I=0.5  ,     #infection rate
               gz=0.05 ,    #grazing rate 
...
b1<-ggplot()+ geom_path(data=solutiondf1, aes(x=time, y=V, col = "0.1 GR"),na.rm = TRUE)+ geom_path(data=solutiondf2, aes(x=time, y=V, col ="0.5 GR"),na.rm = TRUE)+ geom_path(data=solutiondf3, aes(x=time, y=V, col ="1.2 GR"),na.rm = TRUE)+
  xlim(values=c(4700,5000))+
  #ylim(values=c(0,20))+
  labs(x = "Time (d)", y = "Concentration (umol N l-1)",color = "Legend", title= "Virus")

b2<-ggplot()+ geom_path(data=solutiondf1, aes(x=time, y=P, col = "0.1 GR"),na.rm = TRUE)+ geom_path(data=solutiondf2, aes(x=time, y=P, col ="0.5 GR"),na.rm = TRUE)+ geom_path(data=solutiondf3, aes(x=time, y=P, col ="1.2 GR"),na.rm = TRUE)+
  xlim(values=c(4700,5000))+
  #ylim(values=c(0,20))+
  labs(x = "Time (d)", y = "Concentration (umol N l-1)",color = "Legend", title= "Phytoplankton")

b3<-ggplot()+ geom_path(data=solutiondf1, aes(x=time, y=Z, col = "0.1 GR"),na.rm = TRUE)+ geom_path(data=solutiondf2, aes(x=time, y=Z, col ="0.5 GR"),na.rm = TRUE)+ geom_path(data=solutiondf3, aes(x=time, y=Z, col ="1.2 GR"),na.rm = TRUE)+
  xlim(values=c(4700,5000))+
  #ylim(values=c(0,50))+
  labs(x = "Time (d)", y = "Concentration (umol N l-1)",color = "Legend", title= "Zooplankton")


b4<-ggplot()+ geom_path(data=solutiondf1, aes(x=time, y=N, col = "0.1 GR"),na.rm = TRUE)+ geom_path(data=solutiondf2, aes(x=time, y=N, col ="0.5 GR"),na.rm = TRUE)+ geom_path(data=solutiondf3, aes(x=time, y=N, col ="1.2 GR"),na.rm = TRUE)+
  xlim(values=c(4700,5000))+
  #ylim(values=c(0,20))+
  labs(x = "Time (d)", y = "Concentration (umol N l-1)",color = "Legend", title= "Nutrients")

b5<-ggplot()+ geom_path(data=solutiondf1, aes(x=time, y=D, col = "0.1 GR"),na.rm = TRUE)+ geom_path(data=solutiondf2, aes(x=time, y=D, col ="0.5 GR"),na.rm = TRUE)+ geom_path(data=solutiondf3, aes(x=time, y=D, col ="1.2 GR"),na.rm = TRUE)+
  xlim(values=c(4700,5000))+
  #ylim(values=c(0,10))+
