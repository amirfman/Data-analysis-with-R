...
b1<-ggplot()+ geom_path(data=solutiondf1, aes(x=time, y=V, col = "0.1 GR"),na.rm = TRUE)+ geom_path(data=solutiondf2, aes(x=time, y=V, col ="0.5 GR"),na.rm = TRUE)+ 
  xlim(values=c(4700,5000))+
  #ylim(values=c(0,20))+
  labs(x = "Time (d)", y = "Concentration (umol N l-1)",color = "Legend", title= "Virus")

b2<-ggplot()+ geom_path(data=solutiondf1, aes(x=time, y=P, col = "0.1 GR"),na.rm = TRUE)+ geom_path(data=solutiondf2, aes(x=time, y=P, col ="0.5 GR"),na.rm = TRUE)+
  xlim(values=c(4700,5000))+
  #ylim(values=c(0,20))+
  labs(x = "Time (d)", y = "Concentration (umol N l-1)",color = "Legend", title= "Phytoplankton")

b3<-ggplot()+ geom_path(data=solutiondf1, aes(x=time, y=Z, col = "0.1 GR"),na.rm = TRUE)+ geom_path(data=solutiondf2, aes(x=time, y=Z, col ="0.5 GR"),na.rm = TRUE)+
  xlim(values=c(4700,5000))+
  #ylim(values=c(0,50))+
  labs(x = "Time (d)", y = "Concentration (umol N l-1)",color = "Legend", title= "Zooplankton")


b4<-ggplot()+ geom_path(data=solutiondf1, aes(x=time, y=N, col = "0.1 GR"),na.rm = TRUE)+ geom_path(data=solutiondf2, aes(x=time, y=N, col ="0.5 GR"),na.rm = TRUE)+
  xlim(values=c(4700,5000))+
  #ylim(values=c(0,20))+
